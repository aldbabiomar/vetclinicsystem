...
-- One draft per day (audit B20): two people pressing Start Audit at once each
-- found no draft and each created one. get_or_create_draft_session() inserts
-- with ON CONFLICT against this index.
CREATE UNIQUE INDEX audit_sessions_one_draft_per_day ON audit_sessions (audit_date) WHERE status = 'Draft';
...
CREATE TABLE inventory_transactions (
    ...
    -- Why stock moved: a POS sale, a restocked refund, or Consignment
    -- receiving / shrinkage / return. (Audits do not write here: a confirmed
    -- count IS the new baseline.) The comment used to name two reasons, one
    -- of which nothing ever wrote.
    reason TEXT NOT NULL CHECK (reason IN ('sale','refund','consignment_receipt','shrinkage','consignment_return')),
    ref_id TEXT,
    timestamp TIMESTAMPTZ NOT NULL,
    user_id INTEGER,
    FOREIGN KEY (item_id) REFERENCES inventory_list(id),
