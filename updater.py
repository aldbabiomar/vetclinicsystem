...
Releases. Never touches vetclinicsystem-data/ except to read/write
...
setup.py's --enable-updates) — VETCLINICSYSTEM_DATA_DIR and
VETCLINICSYSTEM_RELEASES_DIR must both be set. is_configured() is False
...
def _run_backup():
    ...
    no exceptions. Uses a dedicated folder under vetclinicsystem-data/
...
def _request_restart(delay=2.0):
    ...
    supervisor loop (see Start VetClinicSystem.command / .bat) restart
