...
def test_two_payouts_cannot_together_take_more_than_the_drawer(client, db, sellable):
    ...
    try:
        db.execute("SELECT pg_advisory_xact_lock(%s, %s)", (sales_routes.DRAWER_LOCK_NAMESPACE, today.toordinal()))
        db.execute("INSERT INTO cash_register_payouts (payout_date, amount, reason, logged_by, created_at) "
                   "VALUES (%s,%s,%s,%s,now())", (today, drawer - D("1.000"), "B15 first payout", ADMIN_ID))
        ...
        taken = db.execute("SELECT COALESCE(SUM(amount), 0) s FROM cash_register_payouts "
                           "WHERE payout_date=%s AND reason LIKE %s", (today, "B15 %")).fetchone()["s"]
        ...
        db.execute("DELETE FROM audit_log WHERE table_name='cash_register_payouts' AND record_id IN "
                   "(SELECT id::text FROM cash_register_payouts WHERE reason LIKE %s)", ("B15 %",))
        db.execute("DELETE FROM cash_register_payouts WHERE reason LIKE %s", ("B15 %",))
        db.commit()
...
def test_two_refunds_of_different_lines_cannot_together_exceed_the_sale(client, db, sellable):
    ...
    sale_id = db.execute(
        ...
        "VALUES (now(),%s,%s,0,%s,'Cash',%s) RETURNING id", (ADMIN_ID, D("20.000"), D("15.000"), D("5.000"))).fetchone()["id"]
    line_a, line_b = [db.execute(
        "INSERT INTO sale_items (sale_id, item_id, quantity, unit_price, line_total, discountable) "
        "VALUES (%s,%s,1,%s,%s,true) RETURNING id", (sale_id, inv, D("10.000"), D("10.000"))).fetchone()["id"] for _ in "ab"]
    db.commit()
    try:
        ...
        refund_id = db.execute(
            ...
            "processed_by, created_at) VALUES ('retail', %s, %s, false, %s, 'B15 first refund', 'Cash', %s, now()) "
        ...
        db.execute("INSERT INTO refund_items (refund_id, item_id, quantity, unit_price, line_total, sale_item_id) "
                   "VALUES (%s,%s,1,%s,%s,%s)", (refund_id, inv, D("10.000"), D("10.000"), line_a))
        ...
        db.commit()
        t.join(15)
        refunded = db.execute("SELECT COALESCE(SUM(amount), 0) s FROM refunds WHERE sale_id=%s", (sale_id,)).fetchone()["s"]
