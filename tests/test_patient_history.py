...
def pet(db):
    o, p = _uid("O"), _uid("P")
    db.execute("INSERT INTO owners (id, name) VALUES (%s,%s)", (o, "History Owner"))
    db.execute("INSERT INTO patients (id, owner_id, animal_name) VALUES (%s,%s,%s)", (p, o, "History Pet"))
    db.commit()
    made = {"visits": [], "cases": []}

    def visit(days_ago, visit_type, complaint):
        v = _uid("V")
        db.execute("INSERT INTO visits (id, patient_id, date, case_status, visit_type, complaint) VALUES (%s,%s,%s,%s,%s,%s)",
    ...
    for c in made["cases"]:
        db.execute("DELETE FROM inpatient_cases WHERE id=%s", (c,))
...
def test_the_history_page_shows_the_stay_once(client, db, pet):
    ...
    db.execute("UPDATE inpatient_cases SET complaint=%s WHERE id=%s", ("Distinctive complaint P4", case))
    db.commit()
...
def test_an_admission_records_its_findings_items_and_vets_in_the_change_log(client, db, pet):
    ...
    case = db.execute("SELECT id FROM inpatient_cases WHERE patient_id=%s ORDER BY id DESC LIMIT 1",
                      (pet["id"],)).fetchone()["id"]
    try:
        logged = {r["field"]: r["new_value"] for r in db.execute(
            "SELECT field, new_value FROM audit_log WHERE table_name='inpatient_cases' AND record_id=%s AND action='update'",
            (str(case),)).fetchall()}
        assert logged.get("exam_findings") == "P9 findings" and logged.get("admitted_items") == "P9 lead", logged
    finally:
        db.execute("DELETE FROM audit_log WHERE table_name='inpatient_cases' AND record_id=%s", (str(case),))
        db.execute("DELETE FROM inpatient_cases WHERE id=%s", (case,))
        db.commit()
