...
def test_new_client_pet_and_visit_are_created_together(client, db, registered):
    ...
    owner = db.execute("SELECT * FROM owners WHERE name=%s", (owner_name,)).fetchone()
    assert owner is not None, "the owner was not created"
    registered["owners"].append(owner["id"])
    patient = db.execute("SELECT * FROM patients WHERE animal_name=%s", (animal,)).fetchone()
    ...
    visits = db.execute("SELECT * FROM visits WHERE patient_id=%s", (patient["id"],)).fetchall()
    for v in visits:
...
def a_visit(db):
    ...
    db.execute("INSERT INTO owners (id, name) VALUES (%s,%s)", (o_id, f"WF Owner {o_id}"))
...
def _current_updated_at(db, visit_id):
    row = db.execute("SELECT updated_at FROM visits WHERE id=%s", (visit_id,)).fetchone()
...
def test_visit_edit_rejects_a_malformed_date(client, db, a_visit):
    ...
    original = db.execute("SELECT date FROM visits WHERE id=%s", (a_visit["visit_id"],)).fetchone()["date"]
    ...
    row = db.execute("SELECT date FROM visits WHERE id=%s", (a_visit["visit_id"],)).fetchone()
...
def test_visit_edit_rejects_an_out_of_range_bcs(client, db, a_visit):
    ...
    row = db.execute("SELECT bcs FROM visits WHERE id=%s", (a_visit["visit_id"],)).fetchone()
...
def test_inventory_item_can_be_created(client, db, catalog_cleanup):
    ...
    row = db.execute("SELECT * FROM inventory_list WHERE name=%s", (name,)).fetchone()
...
def test_a_confirmed_audit_is_what_gives_an_item_a_stock_figure(client, db, catalog_cleanup):
    ...
    db.execute("INSERT INTO audit_session_lines (session_id, item_id, stock_counted, received_since_prior) "
               "VALUES (%s,%s,%s,%s)", (sid, row["id"], Decimal("25.000"), Decimal(0)))
    db.commit()
    try:
...
def test_usernames_cannot_be_duplicated(client, db, user_cleanup):
    ...
    for _ in range(2):
        client.post("/admin/users/new", data={
            ...
            "password": "TestPass12345!", "role_id": role["id"], "capmode": "role"},
            follow_redirects=False)
    rows = db.execute("SELECT * FROM users WHERE username=%s", (username,)).fetchall()
    for r in rows:
...
def test_a_stale_edit_is_refused_rather_than_overwriting(client, db, a_visit):
    ...
    saved = db.execute("SELECT * FROM visits WHERE id=%s", (a_visit["visit_id"],)).fetchone()
    ...
    assert resp.status_code != 500
    after = db.execute("SELECT * FROM visits WHERE id=%s", (a_visit["visit_id"],)).fetchone()
...
def test_bulk_edit_rejects_a_negative_cost(client, db, catalog_cleanup):
    ...
    after = db.execute("SELECT * FROM inventory_list WHERE id=%s", (row["id"],)).fetchone()
...
def test_a_visit_cannot_record_a_negative_weight(client, db, a_visit):
    ...
    row = db.execute("SELECT weight_kg FROM visits WHERE id=%s", (a_visit["visit_id"],)).fetchone()
...
def test_an_inpatient_admission_cannot_record_a_negative_weight(client, db, a_visit):
    ...
    if after > before:
        ...
        try:
            ...
            db.execute("DELETE FROM inpatient_cases WHERE id=%s", (row["id"],))
            db.commit()
...
def test_consignment_bulk_edit_rejects_a_negative_cost(client, db, catalog_cleanup):
    ...
    if dist is None:
        did = _uid("D")
        db.execute("INSERT INTO distributors (id, name) VALUES (%s,%s)", (did, f"Test Dist {did}"))
        db.commit()
    else:
        did = dist["id"]
    client.post("/inventory-catalog/new", data={
    ...
    try:
        if owner is not None:
            ...
            if patient is not None:
                rows = db.execute("SELECT weight_kg FROM visits WHERE patient_id=%s",
                                  (patient["id"],)).fetchall()
                for r in rows:
        ...
        if owner is not None:
            patient = db.execute("SELECT * FROM patients WHERE animal_name=%s", (animal,)).fetchone()
            if patient is not None:
                db.execute("DELETE FROM visits WHERE patient_id=%s", (patient["id"],))
                db.execute("DELETE FROM patients WHERE id=%s", (patient["id"],))
            db.execute("DELETE FROM owners WHERE id=%s", (owner["id"],))
            db.commit()
