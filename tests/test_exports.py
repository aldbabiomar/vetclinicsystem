...
def test_a_patient_with_no_visits_still_exports(client, db):
    ...
    db.execute("INSERT INTO owners (id, name) VALUES (%s,%s)", (o_id, f"Empty Owner {o_id}"))
...
def completed_sale_for_receipt(client, db):
    ...
    db.execute("INSERT INTO audit_session_lines (session_id, item_id, stock_counted, received_since_prior) "
               "VALUES (%s,%s,%s,%s)", (session_id, inv_id, 50.0, 0.0))
    db.commit()
...
def test_an_itemised_line_prints_its_quantity_and_line_total(client, db, monkeypatch):
    ...
    db.execute("INSERT INTO owners (id, name) VALUES (%s,%s)", (o_id, f"PDF Owner {o_id}"))
    db.execute("INSERT INTO patients (id, owner_id, animal_name) VALUES (%s,%s,%s)", (p_id, o_id, f"PDF Pet {p_id}"))
    ...
    db.execute("INSERT INTO price_list (id, name, category, cost_price, sale_price, active, can_discount) "
               "VALUES (%s,%s,%s,%s,%s,%s,%s)", (pl_id, name, "Service", D("4.000"), D("12.000"), True, True))
    db.commit()
    tables = []
    ...
    try:
        ...
        for sql, args in (("DELETE FROM visit_billing_lines WHERE visit_id=%s", (v_id,)),
                          ("DELETE FROM billing WHERE visit_id=%s", (v_id,)),
                          ("DELETE FROM visits WHERE id=%s", (v_id,)),
                          ("DELETE FROM price_list WHERE id=%s", (pl_id,)),
                          ("DELETE FROM patients WHERE id=%s", (p_id,)),
