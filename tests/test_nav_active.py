...
app.py (now the main blueprint) kept theirs. nav_active() takes full names and refuses unknown ones.
"""
import re
...
# (page, the sidebar link's href) — one per blueprint.
PAGES = [
    ("/visits", "/visits"),
    ("/pos", "/pos"),
...
def test_nav_active_refuses_a_name_that_is_not_an_endpoint(flask_app):
    ...
    from vcs.web.templating import nav_active
    with flask_app.test_request_context("/visits"):
        with pytest.raises(ValueError):
            nav_active("visits_list")
...
def test_control_nav_active_matches_a_full_endpoint(flask_app):
    ...
    with flask_app.test_request_context("/visits"):   # pushing it matches the URL: request.endpoint is set
        assert nav_active("clinical.visits_list") == "active"
        assert nav_active("sales.pos_page") == ""
