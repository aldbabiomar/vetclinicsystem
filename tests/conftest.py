...
def new_id():
    """A record id for a test fixture that inserts rows directly. Far above
    anything an identity sequence reaches in a test database, so it never
    collides with a row the app itself creates (the columns are GENERATED BY
    DEFAULT, which accepts an explicit id without advancing the sequence)."""
    import random
