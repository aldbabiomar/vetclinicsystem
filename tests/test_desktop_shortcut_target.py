...
vetclinicsystem-releases/app_vX.Y.Z/ snapshot. The next update flips
...
The only correct target is the supervisor launcher in vetclinicsystem-data/,
...
def _managed_install(root):
    ...
    data_dir = os.path.join(root, "vetclinicsystem-data")
    release_dir = os.path.join(root, "vetclinicsystem-releases", "app_v9.9.9")
...
def sandbox(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as root:
        ...
        os.makedirs(desktop)
        bundle = os.path.join(desktop, "VetClinicSystem.app")
...
def test_running_from_a_release_folder_still_targets_the_data_dir(sandbox, monkeypatch):
    ...
    assert "vetclinicsystem-releases" not in line, (
...
def test_running_from_the_original_checkout_also_targets_the_data_dir(sandbox, monkeypatch):
    ...
    checkout = os.path.join(sandbox["root"], "vetclinicsystem-main")
